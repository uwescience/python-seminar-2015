...
## A few recommendations for optimizing Python code (based on http://people.duke.edu/~ccc14/sta-663/Optimization_Bakeoff.html)
...
- Moving to native code:
  ...
  - JIT compilation with numba is improving fast and is already competitive with Cython in the near future. But: you need to write code *for numba*. Sometimes that's actually easier!
...
- If appropriate, consider parallelization (e.g. through dask, to be featured in another session)
- As you optimize your code, remmeber:
  - Check that is is giving correct results!
  - Profile often - it is very hard to predict the effect of an optimizaiton in general
  - Remember that programmer time is also precious - stop when fast enough
